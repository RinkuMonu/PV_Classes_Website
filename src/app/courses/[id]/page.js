...
export default function CourseDetailsPage() {
  ...
  try {
    ...
    if (error.response?.status === 401) {
      // handle silently (don't log)
      // router.push("/login"); // optional
      return null; // consume error
    } else if (error.response?.status === 403) {
      setHasPurchased(false);
    }
    // ✅ yahan bhi kuch log/throw mat karo
    return null;
  } finally {
    setCheckingAccess(false);
  }
  ...
  const calculateTotalPrice = () => {
    ...
    // If all combo items are selected, use the combo discount price
    if (selectAll && course.comboId && course.comboId.discount_price) {
      total += course.comboId.discount_price;
    } else {
      // Otherwise, calculate the sum of individual items
      selectedComboItems.forEach(index => {
        if (course.comboItems && course.comboItems[index]) {
          total += course.comboItems[index].price;
        }
  ...
  const calculateDiscount = () => {
    if (!course || !course.comboId) return 0;
    
    // Calculate the total of all combo items without discount
    let originalComboPrice = 0;
    if (course.comboItems) {
    ...
    return originalComboPrice - course.comboId.discount_price;
  };
