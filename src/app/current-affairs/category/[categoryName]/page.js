...
export default function CategoryAffairsPage() {
  ...
  // Fetch affairs based on selectedCategory
  useEffect(() => {
    const fetchAffairs = async () => {
      try {
        const queryParams = { status: "published", latest: true };
        
        if (selectedCategory && selectedCategory !== "all") {
          queryParams.category = selectedCategory; // ✅ send slug, not name
        }
        if (selectedTag) queryParams.tags = selectedTag;
        if (startDate) queryParams.startDate = startDate;
        if (endDate) queryParams.endDate = endDate;

        const res = await axiosInstance.get("/current-affairs", { params: queryParams });
        setAffairs(res.data);
        setCurrentPage(1);
      } catch (error) {
        console.error("Error fetching current affairs", error);
  ...
  return (
   <>
     ...
     <div className="bg-gray-50 min-h-screen px-3 md:px-20 pb-4">
       <div className="container mx-auto px-4 py-6 flex justify-between items-center">
         <h1 className="text-2xl font-bold text-[#00316B] capitalize">
           {categories.find(c => c.slug === decodedCategorySlug)?.name || "All"} Current Affairs
         </h1>
         <button
           onClick={() => setFilterOpen(true)}
           className="flex items-center gap-2 px-4 py-2 bg-[#00316B] text-white rounded-full hover:bg-blue-800 transition cursor-pointer"
