...
export default function Header() {
  ...
  const total = cart?.reduce((sum, item) => sum + (item?.details?.discount_price || 0) * (item?.quantity || 1), 0)
  ...
  return (
    <>
      {/* Top Bar */}
      <div
        className={`bg-gradient-to-r from-[#00316B] to-[#204972] text-white text-sm border-b border-[#009FE3]/20 transition-all duration-300 py-4 overflow-hidden ${hideTopBar ? "-translate-y-full" : "translate-y-0"}`}
      ...
      >
        <div className="max-w-7xl mx-auto flex justify-between items-center px-4">
          ...
          <div className="flex items-center gap-2">
            <Link
              href=""
              className="relative py-2 px-4 bg-white/10 hover:bg-white/20 rounded-lg transition-all duration-200 inline-flex items-center gap-2 font-medium hover:text-[#009FE3]"
      ...
      <div className="bg-white sticky top-0 z-50 border-b border-[#009FE3]/20 shadow-sm">
        <div className="max-w-7xl mx-auto flex items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            <button className="relative p-3 hover:bg-[#009FE3]/10 rounded-full transition-all duration-200 group">
              <Bell size={20} className="text-[#204972] group-hover:text-[#009FE3]" />
              <span className="absolute top-2 right-2 w-2 h-2 bg-[#87B105] rounded-full animate-pulse"></span>
            </button>
            <button
